...
class Stitcher:
	...
	def stitch(self, images, masks=[None, None], ratio=0.75, reprojThresh=4.0,
		...
		(maskA, maskB) = masks
		(kpsA, featuresA) = self.detectAndDescribe(image=imageA, mask=maskA)
		(kpsB, featuresB) = self.detectAndDescribe(imageB, maskB)
	...
	def detectAndDescribe(self, image, mask=None):
		...
		if self.isv3:
			...
			descriptor = cv2.xfeatures2d.SIFT_create(contrastThreshold=0.04)
